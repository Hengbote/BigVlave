...
static agile_led_t *wifi_G = RT_NULL;       //wifi正常灯
static agile_led_t *wifi_G_Com = RT_NULL;
//static agile_led_t *wifi_R = RT_NULL;       //wifi离线灯
...
void led_Init(void)
{
    if(led0 == RT_NULL)
    {
        ...
        LOG_I("LED_0 Init Success\r\n");
    }

    if(led1 == RT_NULL)
    {
        led1 = agile_led_create(LED0_PIN, PIN_LOW, "200,200", -1);
        LOG_I("LED_1 Init Success\r\n");
    }
    ...
    if(wifi_G == RT_NULL)
    {
        ...
        LOG_I("wifi_G Init Success\r\n");
    }
//    if(wifi_R == RT_NULL)       //离线改闪
//    {
...
//        agile_led_stop(wifi_G);
    ...
    case 3://正在通讯
        agile_led_stop(wifi_G);
        agile_led_set_light_mode(wifi_G, "200,50", 1);
        if(Gateway_ID)
        {
